...
project = rf.workspace().project("tomato-leaf-detector-v3")
model = project.version(1).model

input_folder = 'Images/Captured'
output_folder = 'Images/AuditLeaf'
...
collection_name = "leafParam"  # Replace with your collection name
...
def process_image(image_path):
    ...
    with open(os.path.join(text_data_folder, 'Leaf-Prediction_data.txt'), 'a') as file:
        image_name = os.path.basename(image_path)
        predictions = prediction.json()['predictions']
        for pred in predictions:
            width = pred['width']
            height = pred['height']
            file.write(f"{image_name}, {width}, {height}, {timestamp}, {month_date}, Leaf\n")  # Add comma after timestamp and date
        file.write("------\n")  # Add separator after predictions for each image

    # Upload the text file to MongoDB
    with open(os.path.join(text_data_folder, 'Leaf-Prediction_data.txt'), 'rb') as file:
        file_data = file.read()
        existing_file = collection.find_one({"filename": "Leaf-Prediction_data.txt"})
        if existing_file:
            collection.update_one({"filename": "Leaf-Prediction_data.txt"}, {"$set": {"data": file_data}})
            print("Text file updated in MongoDB")
        else:
            collection.insert_one({"filename": "Leaf-Prediction_data.txt", "data": file_data})
            print("Text file uploaded to MongoDB")

prediction_data_file = os.path.join(text_data_folder, 'Leaf-Prediction_data.txt')
